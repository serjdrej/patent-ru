...
- **Дисциплина ключа.** `ROSPATENT_API_KEY` читается только из переменной окружения. Никогда не печатать, не логировать, не сохранять в JSON/отчётах/коде. Нет переменной — сообщить об этом и предложить запуск в терминале, а не запрашивать ключ в переписке.
...
| Скрипт | Роль |
|---|---|
| `scripts/rospatent.py` | клиент API Роспатента: `search`/`doc`/`similar`/`datasets`, ретраи, три-счёт |
