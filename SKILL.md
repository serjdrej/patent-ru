...
1. **Это ещё один из четырёх актов реестра (ГК РФ ч.4, приказ №107, Регламент
   ...
   обкатан скриптом так, как `pravo_lookup.py` обкатан там: если WebFetch не
   достучится до rospatent.gov.ru, сказать об этом честно, а не молчать.
   **Важно:** открытие страницы подтверждает, что акт существует и какая у
   ...
   срок или формулировку, если вы не прочитали именно нужный абзац.
   Пометка обязана разделять эти два уровня: **AD HOC — акт подтверждён
   живым поиском по маршруту раздела 5 `<дата>`, дата редакции на странице —
   ⟦указать⟧; конкретное содержание нормы ниже — не дословная выдержка,
   сверить лично перед использованием.** Если тема повторится — предложить
