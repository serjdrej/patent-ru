...
#
# Файл хранится ВНЕ дерева скилла — в %LOCALAPPDATA%\rospatent\rospatent_key.enc
# — намеренно: сам скилл может быть установлен и переустановлен куда угодно
# (библиотека навыков, другая папка, другая копия репозитория), а ключ должен
# оставаться на месте независимо от этого. Настройка нужна один раз на
# компьютер, а не один раз на установку скилла.
#
# Что это даёт: файл на диске — не текст ключа, а шифротекст. Расшифровать
# его может ТОЛЬКО эта же учётная запись Windows на ТОЛЬКО этом же компьютере
# (DPAPI использует ключ, производный от вашего логина Windows). Если файл
# случайно попадёт в бэкап, в облачную синхронизацию, на флешку, в скриншот
# каталога — сам по себе он бесполезен.
#
...
#
# На macOS используйте вместо этого rospatent-set-key.sh (хранит ключ в Keychain).

$ErrorActionPreference = "Stop"

$secretsDir = Join-Path $env:LOCALAPPDATA "rospatent"
...
Write-Host "Расположение не привязано к установке скилла — переустановка/перенос скилла ключ не затронет."
